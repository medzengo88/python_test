...
# Создаем лог файл со всеми страницами, которые будем посещать
...
# Вводим URL адреса стартовую и конечную
url1 = input('Введите стартовую ссылку: ')
url2 = input('Введите конечную ссылку: ')
...
try:
    # Заходим на сайт и увеличиваем окно браузера, ждем 2 секунды пока страница загрузится
    driver.get(url=url1)
    logger.info(url1)
    driver.maximize_window()
    time.sleep(2)

    # Ищем элементы, если их не нашли в течении 10 секунд, то произойдет исключение, и так 3 раза
    try:
        driver.maximize_window()
